...
symbols = ones(N_s,1);
symbols = [1;0.5;0;0.5;1;0.5;0;0.5;1;0.5;0;0.5;1;0.5];
for index = 1:14
    symbols(index) = index/14 + j*index/14;
end
symbols = ones(N_s,1)+j*ones(N_s,1);
for index = 1:14
    symbols(index) = index/14 + j*(15-index)/14;
end

index = 1;
for k=-floor(N_s/2):ceil(N_s/2)-1
    cw_waveform_matrix(1:length(t_symbol),k+offset) = symbols(index)*exp(j*2*pi*(fc+k/T_symbol)*t_symbol).';
    index = index + 1;
end
...
% fast-time matched filter data structure
y_mf_mm = zeros(N_sps*2, num_iter);

% define the fast-time matched filter
mf = cw_waveform(N_sps+1:-1:1);
mfcoeff = conj(mf);
h = mfcoeff;
...
%-------------------------------------------------------------------------%
    ...
    % calculate the fast-time matched-filter output
    temp = conv(y_theo(:,i), h);
    y_mf_mm(:, i) = temp(1:N_sps*2);
