...
% daniel: the waveform has length NSampPerPulse = length(x)
%{
x = wav();
plot(1:length(x), x);
%}
...
% daniel:
% combine the returned signal for each signal in each column of ymf
% the matched filter is all ones so applying it to each of the signals
% has the effect of averaging them all out
