...
for i = 1:num_iter
    ...
    for t = (0:1/fs:pulse_period)
        res = 0;
        %blade_ang = blade_init_offset_angle + (time+t)*rps*[1;-1;1;-1];
        for k=-floor(N_s/2):ceil(N_s/2)-1
            for p=1:N_prop
                omega = blade_ang(p);
                x_pi_u = [mag_x_pi_u*cos(prop_init_offset_angle(p));y;...
                ...
                sn = 0;
                for b=1:N_blades
                    q = 220*vel_fac*rps*R_u_o*R_pi_u*[-sin(omega);cos(omega);0];
                    l = 4*pi*(fc+k/T_symbol)/c*(dot(q, x_pi_o/norm(x_pi_o)))*(t-2*norm(x_pi_o)/c);
                    
                    sn = sn + exp(1j*(L1+L2)*l/2)*sinc((L2-L1)*l/2);
                    omega = omega + 2*pi/N_blades;
                end
                res = res + exp(1j*2*pi*(fc+k/T_symbol)*t)*exp(-1j*4*pi*(fc+k/T_symbol)*norm(x_pi_o)/c)*sn; %
                    
                %theo_rcs_ratio*RCS_bsp/RCS_static*exp(1j*2*pi*(fc+k/T_symbol)*(t-2*R/c));
                vel_fac = vel_fac*-1;
            end
            res = res +  exp(1j*2*pi*(fc+k/T_symbol)*(t-2*R/c));
        end
        y_theo(idx, i) = res;
        idx = idx + 1;
    end
        ...
        for k=-floor(N_s/2):ceil(N_s/2)-1
            for p=1:N_prop
                ...
                for b=1:N_blades
                    ...
                    l = -8*pi*(fc+k/T_symbol)/c^2*(dot(q, x_pi_o));
                    l_fac = 1/((L2-L1)*l/2);
                    sn = sn + exp(1j*(L1+L2)*l/2)*sinc((L2-L1)*l/2);
                    %sn = sn + exp(1j*(L1+L2)*l/2)*sinc((L2-L1)*l/2);
                    %sn = sn + 1/(1j*l)*(exp(1j*L2*l)-exp(1j*L1*l));
                    omega = omega + 2*pi/N_blades;
                end
                %FSPL = (4*pi*(fc+k/T_symbol)*norm(x_pi_o)/c)^2;
                %A = 1;%(L2-L1)/(FSPL)*1e8;
                res = res + exp(1j*2*pi*(fc+k/T_symbol)*(t-2*norm(x_pi_o)/c+2/c*t))*sn + ... 
    ...
    %for t = (2*R/c:1/fs:T_symbol+2*R/c)
        ...
        for k=-floor(N_s/2):ceil(N_s/2)-1
            for p=1:N_prop
                ...
                % sum over n 
                sn = 0;
                % find another way to tune the model without a velocity
                % factor
                for b=1:N_blades
                    x_l2_pi = L2*[cos(omega);sin(omega);0];
                    x_l2_o = R_u_o*R_pi_u*x_l2_pi+x_pi_o;

                    q = 110*vel_fac*rps*R_u_o*R_pi_u*[-sin(omega);cos(omega);0];
                    l = 4*pi*(fc+k/T_symbol)/c*(t-2*norm(x_l2_o)/c)*(dot(q, x_l2_o/norm(x_l2_o)));
                    sn = sn + exp(1j*(L1+L2)*l/2)*sinc((L2-L1)*l/2);

                    %sn = sn + exp(1j*(L1+L2)*l/2);%sinc((L2-L1)*l/2);
                    %sn = sn + exp(1j*(L1+L2)*l/2)*sinc((L2-L1)*l/2);
                    %sn = sn + 1/(1j*l)*(exp(1j*L2*l)-exp(1j*L1*l));
                    %sn = sn - 1j/l*exp(1j*L2*l);
                    omega = omega + 2*pi/N_blades;
                end
                %FSPL = (4*pi*(fc+k/T_symbol)*norm(x_pi_o)/c)^2;
                %A = 1;%(L2-L1)/(FSPL)*1e8;
                res = res + exp(1j*2*pi*(fc+k/T_symbol)*(t-2*R/c))*sn + exp(1j*2*pi*(fc+k/T_symbol)*(t-2*R/c));
                %theo_rcs_ratio*RCS_bsp/RCS_static*exp(1j*2*pi*(fc+k/T_symbol)*(t-2*R/c));
                vel_fac = vel_fac*-1;
            end
        end
    ...
    %for t = (2*R/c:1/fs:T_symbol+2*R/c)
        ...
        res = res + RCS_static*RRE(txPower, G_tx, norm(x_u_o), fc+k/T_symbol, G_rx-noiseFigure)* ...
            exp(1j*2*pi*(fc+k/T_symbol)*(t-2*norm(x_pi_o)/c));
        y_theo(idx, i) = res;
        idx = idx + 1;
    end
