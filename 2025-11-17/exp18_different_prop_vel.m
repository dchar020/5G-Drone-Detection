...
for i = 1:num_iter
    ...
    blade_ang = blade_init_offset_angle + [time*rps_back;-time*rps_front;time*rps_front;-time*rps_back];
    idx = 1;
        ...
        for k=-floor(N_s/2):ceil(N_s/2)-1
            for p=1:N_prop
                ...
                for b=1:N_blades
                    ...
                    res = res + (L2-L1)*RRE(txPower, G_tx, norm(x_l2_o), fc+k/T_symbol, G_rx-noiseFigure)*...
            ...
            res = res + RCS_static*RRE(txPower, G_tx, norm(x_pi_o), fc+k/T_symbol, G_rx-noiseFigure)*...
                    exp(1j*2*pi*(fc+k/T_symbol)*(t-2*R/c))/2; 
        end
        if (2*(R-10.4)/c <= t) & (t <= T_symbol+2*(R+10.4)/c)
            y_theo(idx, i) = res;
        end
        idx = idx + 1;
    end
    ...
    for t = 0:1/fs:pulse_period
        ...
        for k=-floor(N_s/2):ceil(N_s/2)-1
            ...
            res = res + RCS_static*RRE(txPower, G_tx, norm(x_pi_o), fc+k/T_symbol, G_rx-noiseFigure)* ...
                exp(1j*2*pi*(fc+k/T_symbol)*(t-2*norm(x_pi_o)/c))/2;
        end
        if (2*(R-10.4)/c <= t) & (t <= T_symbol+2*(R+10.4)/c)
            y_theo_ps(idx, i) = res;
        end
