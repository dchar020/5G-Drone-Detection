...
BW = 200e3;               % only need a small bandwidth to see shift
fs = BW;              % Sampling frequency (must be at least Nyquist
...
TX_FFT = abs(fftshift(fft(cw_waveform_bb(start_idx:end_idx), N)));
RX_FFT = abs(fftshift(fft(rx_signal_bb(start_idx:end_idx), N)));
...
legend('Transmitted', 'Received', 'Theoretical');
